...
const StudentsForms = () => {
  ...
  const getItems= async ()=>{
    try {
      const res = await axios.get(`http://localhost:3000/student/internship`)
  ...
  const ApproveFile = async (id)=>{
    try {
      const res = await axios.post(`http://localhost:3000/student/internship/approved/${id}` , {responseType:'blob'})
      ...
      <div>
        {items && items.map((item)=>(
          <div key={item._id} style={{marginLeft:'10px'}}>
            ...
            <button onClick={()=>ApproveFile(item._id)} disabled={item.approve}>Approve and Send to Career Center</button>
          </div>
        )
